...
class Acknowledge(Node):
    ...
    def send_initial_pose_with_covariance(self):
        ...
        pwcs.pose.pose.position.z = 0.01
...
def main(args=None):
    ...
    try:
        ...
        node = Acknowledge()
        desired_pose = Pose()
        desired_pose.position.x = 1.86
        desired_pose.position.y = 0.45
        ...
        ini_pose = Pose()
        ini_pose.position.x = -2.1
        ini_pose.position.y = -0.3
        ini_pose.orientation.w = 1.0

        node.send_goal_async(ini_pose, "")
